...
class PersonnelNotifier with ChangeNotifier {
  ...
  })  : _userService = userService ?? GluttexLocator.get<AppUserService>(),
        _storageService =
            storageService ?? GluttexLocator.get<StorageService>();
  ...
  Future<bool> answerInvitation(
      {required int ruleId, required int answer}) async {
    try {
      debugPrint('Answering invitation for ruleId: $ruleId, answer: $answer');

      await _storageService.update(
        "${GluttexConstants.apiBaseUrl}${GluttexConstants.answerStaffInvitationEndpoint}/$ruleId",
        "",
        {"answer": answer},
        {},
      );

      final result = _findRuleAndUserId(ruleId);
      if (result == null) {
        debugPrint("Rule $ruleId not found in privileges");
        return false;
      }

      final (targetRule, targetUserId) = result;
